...
| J13 — Video | Fresh launch with the same local playable fixture: open the seeded C# video, use playback controls, Back; choose the fixture from each picker, remove preview, choose again and Send with caption. | A provisional Pixel 10/API 36 run at `60f1800` observed the C# seed thumbnail/frame, fullscreen local playback, Back dismissal, background/resume survival, real-picker preview/remove and empty-caption send. The modified pinned reference selected the same local fixture through its real picker, rendered preview/remove and sent a second video node. Final source-matched play/pause/seek, repeated cleanup and UI-caption proof remain pending. The run did not rely on the Kotlin remote HLS seed. |
...
The provisional Pixel 10 run used the original 1080x2424 @ 420 dpi viewport,
...
The focused video-send regression enumerated and passed one test on the
initially admitted payload. A fresh lease must verify the final installed
candidate/test hashes, visible play/pause/seek frames, repeated cleanup and a
caption through the UI. The bounded run did not expose seek/mute controls
through accessibility or toggle the reference between system themes. It makes
no timing or performance claim.
