...
internal static partial class ComposeBridges
{
    ...
    // androidx.compose.ui.res.PainterResources_androidKt.painterResource(int id, Composer, int $changed)
    // — this @Composable factory has no defaultable parameters, so its
    // generated bytecode has only `$changed` (no `$default` slot).
    // ComposeBridgeGenerator assumes every signature ends in
    // `...Composer;I*I` (one `I` per `$changed` plus one for `$default`),
    // so this bridge stays hand-written.
    //
    // Returns a NEW local Painter ref the caller is responsible for
    // DeleteLocalRef'ing once it's been handed to the consuming
    // Image/Icon JNI call.
    const string PainterResourceSig =
        "(ILandroidx/compose/runtime/Composer;I)Landroidx/compose/ui/graphics/painter/Painter;";

    static IntPtr s_painterResourcesClass;
    static IntPtr s_painterResourceMethod;

    internal static unsafe IntPtr PainterResource(int id, IComposer composer)
    {
        if (s_painterResourcesClass == IntPtr.Zero)
        {
            s_painterResourcesClass = JNIEnv.FindClass("androidx/compose/ui/res/PainterResources_androidKt");
            s_painterResourceMethod = JNIEnv.GetStaticMethodID(s_painterResourcesClass, "painterResource", PainterResourceSig);
        }

        JValue* args = stackalloc JValue[3];
        args[0] = new JValue(id);
        args[1] = new JValue(((Java.Lang.Object)composer).Handle);
        args[2] = new JValue(0); // $changed
        try
        {
            return JNIEnv.CallStaticObjectMethod(s_painterResourcesClass, s_painterResourceMethod, args);
        }
        finally
        {
