...
internal static partial class ComposeBridges
{
    ...
    // androidx.compose.runtime.SnapshotIntStateKt.mutableIntStateOf(int) ->
    // MutableIntState — the primitive-specialized counterpart to
    // mutableStateOf<Integer>(...). Lets MutableNumberState<int> store
    // the value in a slot that exposes getIntValue()I / setIntValue(I)V,
    // eliminating the per-read Java.Lang.Integer allocation.
    [ComposeBridge(
        Class     = "androidx/compose/runtime/SnapshotIntStateKt",
        JvmName   = "mutableIntStateOf",
        Signature = "(I)Landroidx/compose/runtime/MutableIntState;")]
    internal static partial IntPtr MutableIntStateOf(int value);

    [ComposeBridge(
        Class     = "androidx/compose/runtime/SnapshotLongStateKt",
        JvmName   = "mutableLongStateOf",
        Signature = "(J)Landroidx/compose/runtime/MutableLongState;")]
    internal static partial IntPtr MutableLongStateOf(long value);

    [ComposeBridge(
        Class     = "androidx/compose/runtime/PrimitiveSnapshotStateKt",
        JvmName   = "mutableFloatStateOf",
        Signature = "(F)Landroidx/compose/runtime/MutableFloatState;")]
    internal static partial IntPtr MutableFloatStateOf(float value);

    // Hand-written raw-JNI accessors for MutableIntState / MutableLongState
    // / MutableFloatState. The source generator only emits static-call
    // bridges; instance-method calls on an arbitrary handle (the state
    // instance returned by the factory bridges above) don't fit any of
    // its supported shapes, so the cache-then-call boilerplate lives
    // here. Pattern mirrors ModifierCompanionInstance: lazy FindClass +
    // NewGlobalRef + GetMethodID, then CallIntMethod / CallVoidMethod
    // on the supplied instance handle.

    static IntPtr s_mutableIntState_class;
    static IntPtr s_mutableIntState_getIntValue;
    static IntPtr s_mutableIntState_setIntValue;

    static void EnsureMutableIntState()
    {
        if (s_mutableIntState_class != IntPtr.Zero) return;
        IntPtr local = JNIEnv.FindClass("androidx/compose/runtime/MutableIntState");
        s_mutableIntState_class       = JNIEnv.NewGlobalRef(local);
        s_mutableIntState_getIntValue = JNIEnv.GetMethodID(s_mutableIntState_class, "getIntValue", "()I");
        s_mutableIntState_setIntValue = JNIEnv.GetMethodID(s_mutableIntState_class, "setIntValue", "(I)V");
        JNIEnv.DeleteLocalRef(local);
    }

    internal static int MutableIntStateGetIntValue(IntPtr instance)
    {
        EnsureMutableIntState();
        return JNIEnv.CallIntMethod(instance, s_mutableIntState_getIntValue);
    }

    internal static unsafe void MutableIntStateSetIntValue(IntPtr instance, int value)
    {
        EnsureMutableIntState();
        JValue* args = stackalloc JValue[1];
        args[0] = new JValue(value);
        JNIEnv.CallVoidMethod(instance, s_mutableIntState_setIntValue, args);
    }

    static IntPtr s_mutableLongState_class;
    static IntPtr s_mutableLongState_getLongValue;
    static IntPtr s_mutableLongState_setLongValue;

    static void EnsureMutableLongState()
    {
        if (s_mutableLongState_class != IntPtr.Zero) return;
        IntPtr local = JNIEnv.FindClass("androidx/compose/runtime/MutableLongState");
        s_mutableLongState_class         = JNIEnv.NewGlobalRef(local);
        s_mutableLongState_getLongValue  = JNIEnv.GetMethodID(s_mutableLongState_class, "getLongValue", "()J");
        s_mutableLongState_setLongValue  = JNIEnv.GetMethodID(s_mutableLongState_class, "setLongValue", "(J)V");
        JNIEnv.DeleteLocalRef(local);
    }

    internal static long MutableLongStateGetLongValue(IntPtr instance)
    {
        EnsureMutableLongState();
        return JNIEnv.CallLongMethod(instance, s_mutableLongState_getLongValue);
    }

    internal static unsafe void MutableLongStateSetLongValue(IntPtr instance, long value)
    {
        EnsureMutableLongState();
        JValue* args = stackalloc JValue[1];
        args[0] = new JValue(value);
        JNIEnv.CallVoidMethod(instance, s_mutableLongState_setLongValue, args);
    }

    static IntPtr s_mutableFloatState_class;
    static IntPtr s_mutableFloatState_getFloatValue;
    static IntPtr s_mutableFloatState_setFloatValue;

    static void EnsureMutableFloatState()
    {
        if (s_mutableFloatState_class != IntPtr.Zero) return;
        IntPtr local = JNIEnv.FindClass("androidx/compose/runtime/MutableFloatState");
        s_mutableFloatState_class          = JNIEnv.NewGlobalRef(local);
        s_mutableFloatState_getFloatValue  = JNIEnv.GetMethodID(s_mutableFloatState_class, "getFloatValue", "()F");
        s_mutableFloatState_setFloatValue  = JNIEnv.GetMethodID(s_mutableFloatState_class, "setFloatValue", "(F)V");
        JNIEnv.DeleteLocalRef(local);
    }

    internal static float MutableFloatStateGetFloatValue(IntPtr instance)
    {
        EnsureMutableFloatState();
        return JNIEnv.CallFloatMethod(instance, s_mutableFloatState_getFloatValue);
    }

    internal static unsafe void MutableFloatStateSetFloatValue(IntPtr instance, float value)
    {
        EnsureMutableFloatState();
        JValue* args = stackalloc JValue[1];
        args[0] = new JValue(value);
        JNIEnv.CallVoidMethod(instance, s_mutableFloatState_setFloatValue, args);
    }
}
