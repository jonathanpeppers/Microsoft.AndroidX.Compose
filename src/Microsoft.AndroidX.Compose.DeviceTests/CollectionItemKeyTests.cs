...
#pragma warning restore CS8603
    ...
    public void PagerCount_PublishesWithKeySnapshotAndRestoresUnkeyedLiveBehavior()
    {
        ...
        count = 3;
        Assert.AreEqual(3, state.PageCount, "Public count must remain live outside composition.");
        Assert.AreEqual(2, state.Jvm.PageCount, "Native count must not overtake the rendered key snapshot.");
        state.SetRenderedPageCount(3);
        Assert.AreEqual(3, state.Jvm.PageCount);
        count = 0;
        Assert.AreEqual(0, state.PageCount);
        Assert.AreEqual(3, state.Jvm.PageCount);
        state.SetRenderedPageCount(0);
        count = 2;
        Assert.AreEqual(2, state.PageCount, "New records must be visible when a keyed pager is absent.");
        Assert.AreEqual(0, state.Jvm.PageCount);
        count = 4;
        state.SetRenderedPageCount(null);
        Assert.AreEqual(4, state.PageCount);
        Assert.AreEqual(4, state.Jvm.PageCount);
