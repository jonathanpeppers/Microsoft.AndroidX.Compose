...
/// <c>Microsoft.AndroidX.Compose.ComposeBridges</c> decorated with
...
public sealed class ComposeBridgeGenerator : IIncrementalGenerator
{
        ...
        int continuationSlotIdx)
    {
        ...
        sb.AppendLine("// Generated by Microsoft.AndroidX.Compose.SourceGenerators.ComposeBridgeGenerator.");
        ...
        {
            sb.Append("        int defaults = (int)global::Microsoft.AndroidX.Compose.").Append(enumName).AppendLine(".All;");
            foreach (var p in userParams)
            {
                ...
                {
                    sb.Append("        if (").Append(EscapeIdent(p.Name)).Append(" is not null) defaults &= ~(int)global::Microsoft.AndroidX.Compose.")
                      .Append(enumName).Append('.').Append(member).AppendLine(";");
                }
                else
                {
                    sb.Append("        defaults &= ~(int)global::Microsoft.AndroidX.Compose.").Append(enumName).Append('.').Append(member).AppendLine(";");
                }
            }
            sb.AppendLine();
        }
    ...
    static void EmitUserArgValue(StringBuilder sb, IParameterSymbol p, JniType sigType)
    {
        ...
        {
            sb.Append("global::Microsoft.AndroidX.Compose.ComposeBridges.ModifierHandle(").Append(name).Append(')');
            return;
        }
    ...
    static bool IsModifierType(ITypeSymbol t)
    {
        ...
        {
            if (n.Name == "IModifier" && n.ContainingNamespace?.ToDisplayString() == "Microsoft.AndroidX.Compose")
                return true;
        }
        return false;
    }
