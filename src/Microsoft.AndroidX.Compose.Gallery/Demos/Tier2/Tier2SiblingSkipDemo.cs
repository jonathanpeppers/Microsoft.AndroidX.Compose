...
public static class Tier2SiblingSkipDemo
{
    ...
    public static void Parent()
    {
        ...
        Column(() =>
        {
            ...
            Static("constant input — should never re-run");
    ...
    /// </summary>
    [Composable]
    public static void Static(string label)
    {
        s_staticExec++;
        Text($"Static side: \"{label}\" — exec={s_staticExec}");
    }
    ...
    /// </summary>
    [Composable]
    public static void Ticking(int tickCount)
    {
        s_tickingExec++;
        Text($"Ticking side: count={tickCount}, exec={s_tickingExec}");
    }
}
