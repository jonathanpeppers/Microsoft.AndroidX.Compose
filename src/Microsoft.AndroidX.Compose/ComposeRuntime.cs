...
/// </summary>
public static class ComposeRuntime
{
    /// <summary>
    ...
    static T RememberCore<T>(Func<T> factory, object?[]? keys, int line, string file)
    {
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.Remember<T> must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
    ...
    static T RememberSaveableCore<T>(Func<T> factory, object?[]? keys, int line, string file)
    {
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.RememberSaveable<T> must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
    ...
    static T RememberSaveableWrapper<T>(Func<T> factory, object?[]? keys, IComposer composer)
    {
        ...
        // render. `ComposeRuntime.Remember` opens its own nested replaceable
        ...
        var wrapper = Remember(factory)
            ?? throw new InvalidOperationException(
                $"ComposeRuntime.RememberSaveable<{typeof(T).Name}>: factory returned null.");
        ...
        try
        {
            if (handle == IntPtr.Zero)
                throw new InvalidOperationException(
                    $"ComposeRuntime.RememberSaveable<{typeof(T).Name}>: rememberSaveable returned null.");
        ...
        [CallerFilePath] string file = "")
    {
        ...
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.RememberDraggableState must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
        ...
        [CallerFilePath] string file = "")
    {
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.RememberLazyListState must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
    ...
    public static void SideEffect(Action effect)
    {
        ...
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.SideEffect must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
        ...
        Func<DisposableEffectScope, Action> effect)
    {
        ...
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.DisposableEffect must be called inside a composition.");
        ...
        Func<CancellationToken, Task> body)
    {
        ...
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.LaunchedEffect must be called inside a composition.");
    ...
    /// <c>SnapshotMutationPolicy&lt;T&gt;</c>; Microsoft.AndroidX.Compose currently always
    ...
    /// is the bound <c>global::AndroidX.Compose.UI.Text.Input.TextFieldValue</c>;
    ...
    /// <c>global::AndroidX.Compose.UI.Text.TextRangeKt.TextRange(start, end)</c>.
    ...
    ///
    /// <code>
    /// var clock = ComposeRuntime.ProduceState(
    ///     initialValue: "—",
    ...
    /// <see cref="global::Android.Util.Log.Error(string?, string?)"/> under
    /// the <c>Microsoft.AndroidX.Compose</c> tag rather than becoming unobserved
        ...
        string file)
    {
        ...
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.ProduceState<T> must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
    ...
    /// <c>@Composable</c> — do not call <c>ComposeRuntime.Remember</c> or
    ...
    /// <c>ComposeRuntime.SnapshotFlow(() => listState.FirstVisibleItemIndex)</c>,
    /// to fire analytics or fetch more data when the user scrolls
    /// past a threshold. Pair with <c>ComposeRuntime.LaunchedEffect</c> so
    ...
    /// <see cref="Microsoft.AndroidX.ComposeRuntime.ViewModel"/> for this call site — the C#
    /// parity of Kotlin's
    /// <c>androidx.lifecycle.viewmodel.compose.viewModel&lt;T&gt;(…)</c>.
    /// </summary>
    /// <typeparam name="T">A <see cref="Microsoft.AndroidX.ComposeRuntime.ViewModel"/> subclass.</typeparam>
    /// <param name="factory">
    ...
    /// <c>"net.compose:" + typeof(T).FullName + ":" + file + ":" + line</c>
        ...
        where T : ViewModel
    {
        ...
        var composer = ComposeContext.Current
            ?? throw new InvalidOperationException(
                "ComposeRuntime.ViewModel<T> must be called inside a composition (e.g. inside a SetContent body or a ComposableNode.Render override).");
        ...
        {
            throw new InvalidOperationException(
                "ComposeRuntime.ViewModel<T> requires LocalViewModelStoreOwner to be set. " +
