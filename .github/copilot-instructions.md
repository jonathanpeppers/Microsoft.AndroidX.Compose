...
| CN3009 | `[StateHolder]` invalid: non-`IntPtr` param, combined with `[PainterResource]`, missing/non-identifier `Remember`/`StateType`, zero or multiple compatible static remember bridges, inaccessible writable `Jvm` field, invalid `Bind`/`Unbind`, peer return without shared ownership, invalid `Transform`, or conflicting ownership hooks among shared declarations with the same `Remember` and `StateType`. |
