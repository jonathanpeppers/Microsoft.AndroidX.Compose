...
//
// Jetpack Compose ⇄ ComposeNet API coverage report generator.
//
// Run from repo root:
//
...
//   3. Parses `src/ComposeNet.Compose/PublicAPI.Unshipped.txt` to enumerate
...
const string PublicApiPath   = "src/ComposeNet.Compose/PublicAPI.Unshipped.txt";
    ...
    Dictionary<string, List<CSharpSymbol>> csharpByShort)
{
    ...
    // Modifier.xxx extension functions → covered by a method on ComposeNet.Modifier.
...
static void WriteReport(string path, List<Match> matches, List<CSharpSymbol> csharpSymbols, List<string> moduleOrder)
{
    ...
    sb.AppendLine("# Jetpack Compose ⇄ ComposeNet API coverage");
    sb.AppendLine();
    ...
    sb.AppendLine($"- **Covered by ComposeNet (any kind)**: {coveredAll} (**{pctAll:0.0}%**)");
    sb.AppendLine();
    ...
    // whose parent already matched a Kotlin type (e.g. ComposeNet.Alignment.Horizontal
    ...
    {
        "ComposeNet.Resource",        // Android auto-generated resources class
        "ComposeNet.Compose",         // host for Remember/RememberSaveable/Effect statics
        "ComposeNet.ComposeActivity", // base Activity for SetContent
        "ComposeNet.ComposableNode",  // facade base class
        "ComposeNet.ComposableContainer",
        "ComposeNet.RenderContext",
        "ComposeNet.SourceLocationKey",
    };
    ...
    var csharpOnly = csharpSymbols
        ...
        .Where(s =>
        {
            // FQN like ComposeNet.Alignment.Horizontal → parent = Alignment.
    ...
    sb.AppendLine();
    sb.AppendLine("## ComposeNet-only types (no Kotlin counterpart in scope)");
    sb.AppendLine();
