...
| Adaptive layout         | `composer.CurrentWindowAdaptiveInfo()` → live size class and fold posture, `composer.CalculateListDetailPaneScaffoldDirective()`, `composer.RememberListDetailPaneScaffoldNavigator<T>()`, `ListDetailPaneScaffold<T>` / `NavigableListDetailPaneScaffold<T>`, and lifecycle-aware `WindowInfoTracker` layout collection |
